...
async function deployContract(contractName, contractAddress = null, isAirdropNEON = true) {
    ...
    console.log("\nDeployer balance: " + ethers.formatUnits(await ethers.provider.getBalance(deployer.address), 18) + " NEON")

    let deployerBalance = BigInt(await ethers.provider.getBalance(deployer.address))
    if(deployerBalance < minBalance && isAirdropNEON) {
        await airdropNEON(deployer.address, minBalance - deployerBalance)
    }
    const user = (await ethers.getSigners())[1]
    console.log("\nUser address: " + user.address)
    console.log("\nUser balance: " + ethers.formatUnits(await ethers.provider.getBalance(user.address), 18) + " NEON")
    let userBalance = BigInt(await ethers.provider.getBalance(user.address))
    if(userBalance < minBalance && isAirdropNEON) {
        await airdropNEON(user.address, minBalance - userBalance)
    }

    const otherUser = ethers.Wallet.createRandom(ethers.provider)
    console.log("\nOther user address: " + otherUser.address)
    console.log("\nOther user balance: " + ethers.formatUnits(await ethers.provider.getBalance(otherUser.address), 18) + " NEON")
    if (isAirdropNEON) {
